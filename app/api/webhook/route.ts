...
/* Internal metadata — NOT exported (route exports only HTTP methods) */
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (!body || !body.ticker || !body.alert_type) {
      return NextResponse.json({ error: 'Invalid payload: ticker and alert_type are required' }, { status: 400 })
    }
    ...
    if (!validatePayload(body)) {
      return NextResponse.json({
        ...
        required_fields: {
          ...
          alert_type:  'one of 25 SS BlackBox alert types',
          message:     'string',
        },
      }, { status: 422 })
    }
...
/* ─── GET /api/webhook — health check + docs ────────── */
